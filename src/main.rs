...
fn avl_tree_interface<T: GetExample + Copy + Clone + Debug + PartialOrd + Display + FromStr>(type_value:T){
    ...
    loop {
        ...
        println!("8. Print pre-order traversal of the tree.");
        println!("9. Print post-order traversal of the tree.");
        println!("10. Check if the element(s) exists in the tree.");
        println!("11. Quit.");
        println!("----------------------------------------");
        ...
        match user_choice {
            ...
            Some(8) => {
                ...
                m_avl_tree.print_pre_order();
            },
            Some(9) =>{
                println!("----------------------------------------");
                println!("Post-order traversal: ");
                m_avl_tree.print_post_order();
            },
            Some(10) =>{
                println!("----------------------------------------");
                println!("Please input the node(s) you want to check. Separate by one whitespace.");
                let input = convert_input().unwrap();
                if input.len() == 0 {
                    println!("Please key in node(s)!")
                } else {
                    for i in input {
                        let n = m_avl_tree.search(i);
                        let mut result = false;
                        ...
                        println!("Node: {} Existence: {}", i, result);
                    };
                };
            },
            Some(11) => {
...
fn rb_tree_interface<T: GetExample + Clone + Debug + Display + PartialOrd + FromStr + Copy>(type_value: T){
    ...
    loop {
        ...
        println!("6. Check if the tree is empty.");
        println!("7. Print the tree showing its colors and structure.");
        println!("8. Check each property of the Red-Black tree.");
        println!("9. Update the value of a specific node.");
        println!("10. Check if the element(s) exists in the tree.");
        println!("11. Quit.");
        println!("----------------------------------------");
        let user_choice = handle_input();
        match user_choice {
            ...
            Some(8) => {
                println!("----------------------------------------");
                println!("The current tree satisfies each attribute? {}", m_rb_tree.check_properties());
            },
            Some(9) =>{
                println!("----------------------------------------");
                m_rb_tree.show_in_order_traversal(&|&x: &T| print!("{} \t", x));
                println!("");
                println!("Current tree contains values above.");
                println!("Please input the node you want to update. Separate by one whitespace e.g. 1 2(replace 1 with 2)");
                let input = convert_input().unwrap();
                if input.len() != 2 {
                    eprintln!("Wrong number of input. Try again...");
                } else {
                    m_rb_tree.update_node(input.get(0).unwrap().to_owned(), input.get(1).unwrap().to_owned());
            ...
            Some(10) =>{
                ...
                if input.len() == 0 {
                    ...
                    for i in input {
                        println!("Node: {} Existence: {}", i, m_rb_tree.search_node(i));
                    };
                };
            },
            Some(11) => {
